    ...
    def main_life_loop(self):
        ...
        try:
            while self.ede.alive and self.alive:
                ...
                # LAYER 2: Affective
                # Fear analog: predict damage from previous state
                if self.last_context is not None:
                    # Reservoir state is too high-dim for simple similarity in this demo,
                    # but the plumbing is here.
                    self.nis.compute_anticipatory_signal(self.last_context[:1000])
                ...
                if pressure > 0.2: # Threshold adjusted for demo
